module A2D_intf(clk,rst_n,strt_cnv,cnv_cmplt,chnnl,res,SS_n,SCLK,MOSI,MISO);
  ...
  output SS_n;				// active low SPI secondary select to A2D
  output SCLK,MOSI;			// SPI main signals
  
  typedef enum reg[1:0] {IDLE,CMD,WAIT,RSP} state_t;
  state_t state, nxt_state;
  logic [7:0] wait_cnt;
  
  logic [15:0] rspns;		// data read from SPI interface.  Lower 12-bits form res (result of A2D conv)
  logic [15:0] cmd;
  logic done;
